...
r1_update <- function(Y, c, max_iter = 100) {
  svd_result <- svd(Y)
  ...
  for (i in 1:max_iter) {
    M <- svt(Y - l %*% t(f), c)
    flash_res <- flash_init(Y - M) |> flash_factors_init(init = list(l, f), ebnm_fn = ebnm_point_normal) |> flash_backfit(verbose = 0)
    l <- flash_res$L_pm
    f <- flash_res$F_pm
...
```{r}
res <- r1_update(G, 10, max_iter = 1000)
cor(X, res$l)
```
