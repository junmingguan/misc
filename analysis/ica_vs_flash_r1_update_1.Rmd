...
```{r}
res <- r1_update(G, 0.1, sigma_init=eps, g_init = g_init, fix_param = TRUE, max_iter = 1000)
cor(X, res$l)
```
...
```{r}
res <- r1_update(G, 1, sigma_init=eps, g_init = g_init, fix_param = TRUE, max_iter = 1000)
cor(X, res$l)
```
